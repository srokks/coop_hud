import xmltodict
with open('stringtable.sta', 'r') as f:
    str_xml = f.read()

b = xmltodict.parse(str_xml)
b = b['stringtable']
class genLuaTableFile():

    def __call__(self):

    def _parse_lang_file(self):
        pass
    def _gen_lua_file(self):

lang_table = {}
for e in b:
    temp_a = e
    lang_table.update({e:{}})
for key,val in b['info'].items():
    lang_table['info'].update({key:val})
for a in b['languages']['language']:
    lang_table['languages'].update({int(a['@id']):a})
for cat in b['category']:
    cat_name = cat['@name']
    lang_table['category'].update({cat_name:{}})
    for el in cat['key']:
        lang_table['category'][cat_name].update({el['@name']:el['string']})
    # print(cat['@name'])
    # print(cat['key'][0]['@name'])
    # print(cat['key'][0]['string'])
# print(lang_table['category']['Default']['CONTROLLER_DISCONNECTED_0'][0])
with open('../langAPI_tables.lua', 'w') as f:
    f.write('lang_table = {\n')

    for main_cat in lang_table:
        if main_cat == 'info':
            f.write('\tinfo = {\n')
            for k,v in lang_table['info'].items():
                f.write('\t\t')
                f.write(k[1:]+' = "'+v+'",\n')
            f.write('\t},\n')
        if main_cat == 'languages':
            f.write('\tlanguages = {\n')
            for k, v in lang_table['languages'].items():
                f.write('\t\t')
                f.write('['+str(k)+'] = {\n')
                nn = ['\t\t\t'+kk[1:]+' = "'+vv+'",\n' for kk,vv in v.items()]
                f.writelines(nn)
                f.write('\t\t')
                f.write('},\n')
            f.write('\t},\n')
        if main_cat == 'category':
            f.write('\tcategory = {\n')
            for k,v in lang_table['category'].items():
                f.write('\t\t')
                f.write(k+ ' = {\n')
                for n in v.keys():
                    f.write('\t\t\t')
                    f.write(n+' = {')
                    for el in v[n]:
                        if el is not None:

                            f.write('"'+el+'",')
                    f.write('},\n')
                f.write('\t\t')
                f.write('},\n')
            f.write('\t},\n')
    f.write('}')

getLangTable()