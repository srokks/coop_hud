...
function coopHUD.renderHearts(player, pos, mirrored, scale, down_anchor)
	...
	counter = 0
	if player.has_sub then
		-- Sub player heart render
		for row = 0, n - 1, 1 do
			for col = 0, m - 1, 1 do
...
function coopHUD.renderExtraLives(player, pos, mirrored, scale, down_anchor)
	...
	if player.extra_lives ~= 0 then
		...
		f:DrawStringScaled(text, pos.X + lives_pivot.X, pos.Y + lives_pivot.Y, sprite_scale.X, sprite_scale.Y,
...
function coopHUD.renderPockets(player, pos, mirrored, scale, down_anchor)
	...
	if player.sprites.first_pocket then
		...
		--
		if down_anchor then
			-- defines if anchor for rendering is in in left down corner
		...
		final_offset = offset
		-- Main pocket charge
		if player.sprites.first_pocket:GetDefaultAnimation() == 'Idle' then
			-- checks if item is not pill of card
...
function coopHUD.renderChancesDiff(pos)
	...
	if chances_draw then
		...
		if coopHUD.chances_counter > 200 then
			coopHUD.HUD_table.chances.devil[2] = 0
			coopHUD.HUD_table.chances.angel[2] = 0
			coopHUD.HUD_table.chances.planetarium[2] = 0
			chances_draw = false
		end
	else
		coopHUD.chances_counter = 0
...
function coopHUD.renderItems()
	...
	if coopHUD.signals.map then
		-- Catches if map button is pressed
		f_col.Alpha = 1
	...
	coopHUD.HUD_table.sprites.timer_font:DrawString(time_string,
	                                                ...
	                                                f_col, 1, true)
	---
	if coopHUD.streak_main_line ~= nil then
		-- Triggers animation if global string not nil
		if coopHUD.HUD_table.streak:IsFinished() then
			coopHUD.HUD_table.streak:Play('Text', true)
		end
	end
	-- Renders prompt on start
	if not Game():IsPaused() and secs > 0 then
		-- Prevents showing too early on start
...
function coopHUD.renderStreak(sprite, first_line, second_line, pos, signal)
	...
	if not sprite:IsFinished('Text') and sprite:IsPlaying('Text') then
		-- Renders only when animation
		if first_line then
		...
		if second_line and sec_font then
			...
			sec_font:DrawString(second_line, first_line_pos.X, first_line_pos.Y + 30,
			                    coopHUD.HUD_table.streak_sec_color, 1, true)
		end
	end
end
...
function coopHUD.render()
	...
	if coopHUD.options.onRender and not paused then
		-- Renders HUD if game not paused and option turned on
...
function coopHUD.getStatChangeAttrib(stat, format)
	...
	local dif_string = ''
	dif_string = string.format(str_format, stat)
