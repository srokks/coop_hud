...
def parse_to_table(path):
    lang_tree = parse_lang_file(path)
    lang_table = {}
    for e in lang_tree:
        temp_a = e
        lang_table.update({e: {}})
...
def genLuaTableFile(path):
    lang_table = parse_to_table(path)

    with open('../../langAPI_tables.lua', 'w', encoding="utf-8") as f:
        f.write('--[[\n')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Crappy parser from *.sta lang file from Isaac REP resources to lua table file')
    parser.add_argument(dest='path', type=str, help='path to stringtable.sta')
    args = parser.parse_args()
    genLuaTableFile(args.path)
