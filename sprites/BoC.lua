...
function coopHUD.BoC.calculate(player)
	...
	local result = 0
	if EID then
		if EID.calculateBagOfCrafting then
			local bag = {}
			for _, k in pairs(player.bag_of_crafting) do
				table.insert(bag, k.id)
			end
			local components = { table.unpack(bag) }
			local id_a, id_b = EID:calculateBagOfCrafting(components)
			result = id_a
			item_a = coopHUD.Item(player, -1, id_a)
			item_b = coopHUD.Item(player, -1, id_b)
		end
	end
	return result
end
